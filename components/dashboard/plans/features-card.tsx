...
export function FeaturesCard({ data, updateData, errors }: FeaturesCardProps) {
            ...
            <div className="flex flex-wrap gap-2 pt-2">
                {data.features.map((feature, index) => (
                    <div key={index} className="group flex items-center gap-2 bg-primary/5 text-primary px-4 py-2 rounded-lg text-sm font-medium border border-primary/10 hover:bg-primary/10 transition-colors">
                        {feature}
                        <X className="h-4 w-4 cursor-pointer text-primary/40 group-hover:text-destructive transition-colors" onClick={() => removeFeature(index)} />
                    </div>
                ))}
